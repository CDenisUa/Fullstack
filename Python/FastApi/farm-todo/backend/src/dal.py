...
class ToDoDAL:
    ...
    async def get_todo_list(self, id: str | ObjectId, session=None) -> ToDoList:
        doc = await self._todo_collection.fine_one(
            {"_id": ObjectId(id)},
            session=session,
        )
        return ToDoList.from_doc(doc)

    async def delete_toto_list(self, id: str | ObjectId, session=None) -> bool:
    ...
    async def create_item(self, id: str | ObjectId, label:str, session=None) -> ToDoList | None:
        result = await self._todo_collection.fine_one_and_update(
            {"_id": ObjectId(id)},
            {"$push": {
                "items": {
                    "id": uuid4().hex,
